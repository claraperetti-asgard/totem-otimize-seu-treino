...
const estiloNivel: Record<NivelMedalha, { selo: string; borda: string }> = {
  ouro: { selo: 'bg-amber-200 text-[#1B2436]', borda: 'border-amber-300/60' },
  prata: { selo: 'bg-slate-200 text-[#1B2436]', borda: 'border-slate-300/50' },
  bronze: { selo: 'bg-orange-300 text-[#1B2436]', borda: 'border-orange-400/50' },
}
...
function CardEstatistica({
  ...
  return (
    <div className="rounded-xl border border-[#24334D] bg-[#16233A] p-5 text-center">
...
function CardMedalha({ medalha }: { medalha: Medalha }) {
  ...
  return (
    <div
      className={`flex flex-col items-center rounded-xl border bg-[#16233A] p-5 text-center ${estilo.borda}`}
    >
      <span
      ...
      <span className="mb-3 text-[#C4CEDC]">{iconesMedalha[medalha.icone]}</span>
      <h3 className="text-sm font-extrabold uppercase leading-tight text-[#FAF7F1]">
        {medalha.nome}
      </h3>
  ...
  if (!morador) {
    return (
      <div className="w-full flex-1 bg-[#26303b] px-16 pb-10 pt-10 text-white">
        <button
          onClick={onVoltar}
          className="mb-6 flex h-12 w-12 items-center justify-center rounded-xl bg-[#16233A] text-[#FAF7F1]"
        >
          <ArrowLeft size={24} />
        </button>
  ...
  return (
    <div className="w-full flex-1 bg-[#26303b] px-16 pb-10 pt-10 text-white">
      ...
      <div className="mb-8 flex flex-col items-center">
        <div className="rounded-2xl border-2 border-[#FAF7F1] p-2">
          <Avatar nome={morador.nome} tamanho={96} destacado />
        </div>
        <span className="-mt-3 rounded bg-[#FAF7F1] px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-black">
          {posicao}º no ranking
        </span>
        <h2 className="mt-3 text-4xl font-extrabold uppercase tracking-wide text-[#FAF7F1]">
          {morador.nome}
        </h2>
        ...
        <div className="mt-5 w-full max-w-md rounded-xl border border-[#FAF7F1]/40 bg-[#16233A] p-4">
          <div className="mb-2 flex items-center justify-between">
            <span className="flex items-center gap-2 text-sm font-extrabold uppercase tracking-wide text-[#FAF7F1]">
              <Zap size={16} />
      ...
      <div className="mb-10 grid grid-cols-3 gap-4">
        <CardEstatistica
          ...
          icone={<Flame size={22} className="text-orange-300/70" />}
          cor="text-[#FAF7F1]"
        />
        <CardEstatistica
        ...
        <CardEstatistica
          ...
          icone={<Medal size={22} className="text-amber-300/70" />}
          cor="text-[#FAF7F1]"
        />
      </div>

      {/* Progresso da semana */}
      <div className="mb-10 rounded-xl border border-[#24334D] bg-[#16233A] p-5">
      ...
      <div className="mb-4 flex items-baseline justify-between">
        <div>
          <h2 className="text-lg font-extrabold uppercase tracking-wide text-[#FAF7F1]">
            Medalhas
          </h2>
      ...
      {morador.medalhas.length === 0 ? (
        <div className="mb-10 rounded-xl border border-[#24334D] bg-[#16233A] p-10 text-center text-gray-400">
      ...
      {/* Desempenho recente */}
      <h2 className="mb-4 text-lg font-extrabold uppercase tracking-wide text-[#FAF7F1]">
        Desempenho Recente
      </h2>
      <div className="flex items-center gap-5 rounded-xl border border-[#24334D] bg-[#16233A] p-5">
        <div className="rounded-lg bg-[#FAF7F1] px-5 py-4 text-black">
        ...
        <div className="flex-1">
          ...
          <div className="h-2 w-full overflow-hidden rounded-full bg-white/10">
            <div
              className="h-full rounded-full bg-[#FAF7F1]"
              style={{
                width: `${Math.min(
                  100,
        ...
        <span
          className={`flex items-center gap-1 text-lg font-extrabold ${
            subiu ? 'text-emerald-300/80' : 'text-rose-300/80'
          }`}
        >
